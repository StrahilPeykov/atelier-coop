...
export function spawn(s:GameState,index:number):Transform {
  ...
  if(s.level===5)return transform(s.shape*8+(index===0?-1.4:1.4),1,2);
...
export function initialState(host:string):GameState {
 return {version:0,epoch:0,phase:'lobby',level:0,time:0,shape:-.9,bloom:0,step:0,done:false,flags:[],ride:0,seal:0,ferryZ:6,message:'',players:[{id:host,role:'Shaper',ready:false,transform:transform(-1.6),channel:false,axis:0,lift:0,sparkAt:-99,held:-1}],toys:[{x:-5,y:.1,z:9,owner:null},{x:5,y:.1,z:9,owner:null},{x:4,y:.1,z:4,owner:null}],seed:Math.floor(Math.random()*10000),paused:false};
}
