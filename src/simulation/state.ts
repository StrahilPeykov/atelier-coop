...
export type GameState = {version:number;epoch:number;phase:'lobby'|'playing'|'ending';level:number;time:number;shape:number;bloom:number;step:number;done:boolean;flags:number[];ride:number;seal:number;message:string;players:Player[];toys:{x:number;y:number;z:number;owner:string|null}[];seed:number;paused:boolean};
...
export const flowerPosition = (s:GameState):Vec=>({x:s.shape*9,y:1+s.bloom*6,z:[6,-1,-8,-12][Math.min(3,s.step)]});
...
export function spawn(s:GameState,index:number):Transform {
  if(s.level===4)return transform(index===0?-8:8,0,9);
  if(s.level===5)return transform(index===0?-1.4:1.4,1,2);
...
export function initialState(host:string):GameState {
 return {version:0,epoch:0,phase:'lobby',level:0,time:0,shape:-.9,bloom:0,step:0,done:false,flags:[],ride:0,seal:0,message:'',players:[{id:host,role:'Shaper',ready:false,transform:transform(-1.6),channel:false,axis:0,lift:0,sparkAt:-99,held:-1}],toys:[{x:-5,y:1.3,z:9,owner:null},{x:5,y:1.3,z:9,owner:null},{x:4,y:1.3,z:4,owner:null}],seed:Math.floor(Math.random()*10000),paused:false};
}
