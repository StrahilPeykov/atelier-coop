...
import { type GameState,type Transform,type Role,type GameEvent,flowerPosition,targets,desired,aligned,clamp } from '../simulation/state';
...
export class View {
 ...
 private label(text:string,x:number,y:number,z:number,parent:T.Object3D=this.decor){const c=document.createElement('canvas');c.width=512;c.height=128;const ctx=c.getContext('2d')!;ctx.fillStyle='rgba(34,54,45,.9)';ctx.roundRect(4,8,504,112,12);ctx.fill();ctx.strokeStyle='#c9ac73';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#f2dfb3';ctx.font='36px Georgia';ctx.textAlign='center';ctx.fillText(text,256,77);const texture=new T.CanvasTexture(c);texture.colorSpace=T.SRGBColorSpace;const m=new T.Sprite(new T.SpriteMaterial({map:texture,depthWrite:false}));m.position.set(x,y,z);m.scale.set(5,1.25,1);parent.add(m);this.labelSprites.push(m);return m;}
 build(s:GameState){
  ...
  for(let i=0;i<8;i++){const a=i*2.4,r=43+(i%3)*12,x=Math.sin(a)*r,z=Math.cos(a)*r;const g=new T.Group();this.decor.add(g);g.position.set(x,-10-(i%4)*3,z);mesh(new T.CylinderGeometry(7,2,12,7),0x789080,g,0,0,0);this.arch(g,0,7,12);for(let k=0;k<3;k++)this.plant(g,(k-1)*3,1,.9);}
  for(let i=0;i<18;i++){const a=i*2.4;const cloud=orb(this.decor,0xe3dfc9,Math.sin(a)*60,-17-i%4,Math.cos(a)*60,12,.22);cloud.scale.x*=1.6;cloud.castShadow=false;}
  if([0,3,6].includes(s.level)){
   ...
   this.plant(this.growth,0,0,s.level===6?2:1.2);
  }
  ...
  if(s.level===4){this.label('ROOT GALLERY',8,4,7);this.label('STRUCTURE GALLERY',-8,4,7);for(let i=0;i<3;i++){const g=this.arch(this.puzzle,-7,2.4,6);g.position.x=(i-1)*5;this.label(String(i+1),(i-1)*5,7,-7);this.stageItems.push(g);}for(const x of [-8,8])ring(this.decor,1.4,.05).position.set(x,.05,-4);}
  ...
  this.buildGuide();this.updateSurfaces(s);this.yaw=0;this.pitch=.32;
 }
 ...
 private avatar(index:number,role:Role){const root=new T.Group();this.actors.add(root);let model=this.instance(index===0?'Ivo':'Nell',root,0,0,0) as T.Group|undefined;
  ...
  const glyph=new T.Group();glyph.name='castGlyph';root.add(glyph);ring(glyph,.68,1.3,role==='Shaper'?0xffd09a:0xc2e6b0,.025);glyph.visible=false;
 ...
 update(s:GameState,localId:string,local:Transform|undefined,dt:number,menu:boolean,mouseX=0,mouseY=0){
  ...
  const a=aligned(s),target=targets(s);this.guide.position.set(target.x,target.y-.8,target.z);this.guide.rotation.y=time*.2;
  ...
  if(menu){this.cameraPos.set(9,7.5,16);this.focusPoint.set(-1,2.7,-1);this.camera.position.lerp(this.cameraPos,1-Math.exp(-dt*2));this.camera.lookAt(this.focusPoint);}
