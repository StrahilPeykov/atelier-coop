...
export class View {
 ...
 build(s:GameState){
  ...
  for(let i=0;i<8;i++){const a=i*2.4,r=43+(i%3)*12,x=Math.sin(a)*r,z=Math.cos(a)*r;const g=new T.Group();g.userData.distant=true;this.decor.add(g);g.position.set(x,-10-(i%4)*3,z);mesh(new T.CylinderGeometry(7,2,12,7),0x789080,g,0,0,0);this.arch(g,0,7,12);for(let k=0;k<3;k++)this.plant(g,(k-1)*3,1,.9);}
  ...
  if(s.level===4){this.label('ROOT GALLERY',8,3.5,1);this.label('STRUCTURE GALLERY',-8,3.5,1);for(let i=0;i<3;i++){const g=this.arch(this.puzzle,-7,2.4,6);g.position.x=(i-1)*5;this.label(String(i+1),(i-1)*5,7,-7);this.stageItems.push(g);}for(const x of [-8,8]){ring(this.decor,1.4,.05).position.set(x,.05,-4);mesh(cylGeo,palette.dark,this.decor,x,.55,-4,.7,1.1,.7);ring(this.decor,.7,0).position.set(x,1.13,-4);}}
